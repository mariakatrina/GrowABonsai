...
ctx.fillStyle = 'rgb(89, 68, 63)';  // a color name or by using rgb/rgba/hex values
...
ctx.fillStyle = 'rgb(89, 68, 63)';  // a color name or by using rgb/rgba/hex values
ctx.fillText('Grow a', 197, 100); // text and position

// GROW A BONSAI
ctx.font = 'bold 20px calibri';
ctx.textAlign = 'left';
ctx. textBaseline = 'left';
ctx.fillStyle = 'rgb(33, 80, 36)';  // a color name or by using rgb/rgba/hex values
ctx.fillText('Bonsai.', 263, 100); // text and position
...
ctx. textBaseline = 'left';
ctx.fillStyle = 'rgb(89, 68, 63)';  // a color name or by using rgb/rgba/hex values
ctx.fillText('"From Little Seeds Grow Strong Trees."', 198, 155); // text and position
...
class Root {
    ...
    update () {
        ...
        if (this.lightness < 90) this.lightness += 0.3;
        ...
        if (this.size < this.maxSize){
            ...
            // draws a circle
            ctx.fillStyle = 'hsl(225, 10%,' + this.lightness + '%)';
...
class Flower {
    constructor(x, y, size){
        ...
        this.size > 6 ? this.willFlower = true : this.willFlower = false;
            // ? = give property,  : = else
        this.angle = 0;
            // to rotate flowers
